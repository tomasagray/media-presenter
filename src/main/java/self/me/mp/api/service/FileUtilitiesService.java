...
public class FileUtilitiesService {
	...
	public void repairFilename(@NotNull Path file) throws IOException {
		...
		if (filename.contains(ILLEGAL_FILENAME_CHAR)) {
			final String fixedName = new String(Normalizer.normalize(filename, Normalizer.Form.NFKD)
					.getBytes(StandardCharsets.US_ASCII), StandardCharsets.UTF_8)
			...
			final String reason = renamed && createdDirs ?
