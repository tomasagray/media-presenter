...
public class TranscodingService {
  ...
  private String getTranscodeVideoCodec(FFmpegMetadata metadata) {
    ...
    return codec != null && SUPPORTED_VCODECS.contains(codec) ? "copy" : SUPPORTED_VCODECS.get(0);
  }

  private String getTranscodeAudioCodec(FFmpegMetadata metadata) {
    String codec = getCodec(metadata, "audio");
    return codec != null && SUPPORTED_ACODECS.contains(codec) ? "copy" : SUPPORTED_ACODECS.get(0);
  }
  ...
  private boolean checkCorrectVideoFormat(FFmpegMetadata metadata) {
    ...
    return title != null
        ...
        && videoCodec != null
        && SUPPORTED_VCODECS.contains(videoCodec)
        && audioCodec != null
        && SUPPORTED_ACODECS.contains(audioCodec);
  }
